...
# into a file (input_file_name) whose name is chosen by the user. This file is responsible for doing the inner workings of the script as seen 
...
print("Welcome to the find and replace Python script!")
...
input_file_name = input("What would you like to call the file used within the script? ")
...
# file_word. This change is saved in a txt file with the name set by the user. Note only one word can be changed at a for every time the script is ran. Additional, error checking done to ensure
		...
		if file_word in total_words:	 
			...
			if replace_answer == 1:			
				...
				with open(output_file_name, 'w') as f2:
					...
					print("To replace another word within the same file. Rerun the script with the scan file as", output_file_name)
					print("To scan another file, simply rerun the script")
					break
